...
## Purpose
The player carries ten slots holding blocks and equipment. A slot is drawn with
its item's own thumbnail rather than its name: an inventory presented as a list
of names is the failure this capability's rule exists to prevent, and the block
textures the terrain already ships are what a slot draws.
